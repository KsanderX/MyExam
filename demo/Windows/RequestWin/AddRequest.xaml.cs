    ...
    public partial class AddRequest : Window
    {
        ...
        private void Button_add(object sender, RoutedEventArgs e)
        {
                ...
                !string.IsNullOrWhiteSpace(BoxDelivary.Text))
            {
                try
                {
                    //тут идёт присвоение id как как в таблице не установлен автоикремент для поля ID,
                    Order order = new Order()
                    {
                        Id = context.Orders.Max(q => q.Id) + 1,
        ...
        {
            MessageBox.Show("Вы вышли из окна создания заявки", "Успех!", MessageBoxButton.OK, MessageBoxImage.Information);
            DialogResult = false;
        }
    }
}
